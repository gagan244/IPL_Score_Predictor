...
def load_model(file_path):
    ...
    if not os.path.exists(file_path):
        st.error(f"Model file '{file_path}' not found. Please make sure the file exists in the correct directory.")
        st.stop()
    if os.stat(file_path).st_size == 0:
        st.error(f"Model file '{file_path}' is empty.")
        st.stop()
...
st.markdown(
    ...
    <style>
    .stApp {{
        background-image: url("https://machinelearningprojects.net/wp-content/uploads/2021/07/IPL-Score-Prediction.webp");
...
for i, team in enumerate(teams):
    if batting_team == team:
        prediction_array += [1 if j == i else 0 for j in range(len(teams))]
